<?php
	...
	if (isset($_POST['submit'])) {
						...
						if (!preg_match("/^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z!@#$%][\S+]{8,12}$/",$pass)) {
						  $passErr = "Password does not meet the requirements!";
						  $err3=false;
						}else{
							$passErr = "";
							$err3 = true;
					  ...
					  if (empty($_POST["compass"])) {
						$compassErr = "Confirm password required";
						$err4=false;
					  }
					  ...
					  if (empty($_POST["check"])) {
						...
						$err5=false;
						  ...
						  if (empty($_POST["UserName"])) {
							...
							$s = $_POST['UserName'];
							if (!preg_match("/^(?=.*\d)[0-9A-Za-z!@#$%]*$/",$uname)) {
							  $unameErr = "Invalidd username(letters and numbers Required";
							  $err8=false;
							}    
							else if(strlen($s) < 5){
